...
export class DatabaseService implements OnModuleInit, OnModuleDestroy {
  ...
  getDataSource(): DataSource {
    if (!this.dataSource) {
      ...
      throw new Error(
        'DataSource instance not created. Check database configuration.',
      );
    }

    if (!this.dataSource.isInitialized) {
      this.logger.error(
        'Database connection not initialized. Module may not have started properly.',
      );
      throw new Error(
        'Database connection not initialized. Make sure the DatabaseService.onModuleInit() has been called.',
      );
    }
